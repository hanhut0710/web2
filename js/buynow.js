...
if(selectBtn){
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        fetch('./handle/get_product_detail.php',{
            method : "POST",
            headers : {
                'Content-Type' : 'application/x-www-form-urlencoded'
            },
            body : 'product_detail_id=' + encodeURIComponent(serverData.product_detail_id)
        })
        .then(res => res.json())
        .then( data => {
            data.quantity = 1 ;
            console.log('Thông tin sản phẩm:', data.quantity);
            const li = document.createElement("li");
                  li.innerHTML = `
                      ${String(1).padStart(2, "0")}. 
                      ${data.product_name} 
                      <span>${data.total.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</span>
                  `;
    ...
    if (paymentCheckbox.checked) {
        ...
        type.value = "buy_now"; // or "cart" tùy ngữ cảnh
