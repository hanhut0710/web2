...
$address_str = '';
$cartItems = isset($_POST['buy_now_data']) ? json_decode($_POST['buy_now_data'], true) : null;
  ...
  <?php if ($order): ?>
    ...
    <div class="info-group full-width">
      ...
      <table>
        ...
        <?php
          $total = 0;
          $thanhTien = $cartItems['total'];
          $total += $thanhTien;
        ?>
            <tr>
            <td><img src="<?= htmlspecialchars($cartItems['img_src']) ?>" alt="<?= htmlspecialchars($cartItems['product_name']) ?>" width="200" height="120"></td>
            <td><?= htmlspecialchars($cartItems['product_name']) ?></td>
            <td><?= htmlspecialchars($cartItems['quantity']) ?></td>
            <td><?= number_format($cartItems['total'], 0, ',', '.') ?>₫</td>
